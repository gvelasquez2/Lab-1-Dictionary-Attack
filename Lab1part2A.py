...
# This is part 2A from Assignment # 1. This code is independent and can be ran alone using the command line by using:  python Lab1part2A.py
...
def createUserDataList():
    ...
    passFile = open("UnsaltedPassTable.txt",encoding = "ISO-8859-1") # File with user data 
    for line in passFile.readlines():
        if ":" in line:
            user = line.split(':')[0]
            encryptedPass = line.split(':')[1].strip()
            userDataList.append([user,encryptedPass,index]) # I store the index so that I can sort the data before creating the final output file
            index = index+1 
    return userDataList
...
def dictionaryAttack(userDataArray):
    ...
    crackedPasswordsFile = open("crackedPasswords.txt","w") # used to sort cracked passwords
    
    for line in passwordFile.readlines():
        line = line.strip()
        password = hashlib.md5(line.encode('utf-8')).hexdigest()
        
        for user in userDataArray:
            encryptedPass = user[1]
            
            
            if(password == encryptedPass):
                crackedPasswordsFile.write(str(user[0])+ ":"+str(line) + ":"+ str(user[2])+"\n") # if found append info to the cracked password file
...
def numbersAttack(userDataArray):
    ...
    crackedPasswordsFile = open("crackedPasswords.txt","a") # we will now append to the file since this method is called after another attack
...
def combinedWordsAttack(userDataArray):
    ...
    crackedPasswordsFile = open("crackedPasswords.txt","a")
...
def formatOutput():
    crackedPasswords = open("crackedPasswords.txt",encoding = "ISO-8859-1")
...
print("Password Cracking for Part 2A Complete, Please review Results.txt to see list of cracked passwords")
print ("Program took:", time.time() - start_time, "to run")
# Program takes 3683 seconds to run which is about 1 hour
